...
from tools.common.table_formatter import format_table, ELB_COLUMNS
...
def list_elb(region: str = "") -> str:
    ...
    items = result.data.get("loadbalancers", []) if isinstance(result.data, dict) else []
    table_md = format_table(items, ELB_COLUMNS)
    d = result.to_dict()
    if table_md:
        d["_table"] = table_md
