...
def _build_iam_auth_payload(settings: Settings) -> dict[str, Any]:
    # X-Auth-Token is retrieved from IAM with IAM user + domain credentials.
    # TODO: set your IAM username / password / domain name in .env
    return {
        "auth": {
            "identity": {
                "methods": ["password"],
...
def get_iam_token(settings: Settings) -> str:
    """
    Returns an X-Auth-Token requested from IAM.
    """
...
def _extract_transcript(sis_payload: dict[str, Any]) -> str:
    # Common shape: result.text or result[] depending on account/settings.
...
def transcribe_short_audio(audio_path: Path, settings: Settings) -> dict[str, Any]:
    # project_id must match the selected region and SIS endpoint.
    # TODO: replace with your Huawei Cloud region + project ID values from My Credentials.
    ...
    for property_name in _candidate_properties(settings):
        ...
        # SIS.0032 indicates unsupported/invalid property for this account/region.
